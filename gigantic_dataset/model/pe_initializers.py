...
class RWPE_Initializer(PE_Initializer):
    def forward(self, data: Data) -> Tensor:
        ...
        if data.rw_pe is not None:
            return data.rw_pe.to(device)
        ...
        pe = torch.zeros((num_nodes, self.pe_dim), device=device)
        for graph_id in data.batch.unique(sorted=True):
            ...
            sub_pe = calculate_rwpe(subgraph_edge_index, len(node_indices), self.pe_dim)
...
class GeoPE_Initializer(PE_Initializer):
    ...
    '''
    def __init__(self, coord_dim: int = 2, pe_hidden_dim: int =128, pe_dim: int = 20, pe_base: str = ""):
        '''
        ...
        self.emb_dim = pe_dim
        self.spatial_encoder = GridCellSpatialRelationEncoder(spa_embed_dim=pe_hidden_dim, ffn=True, min_radius=1e-06, max_radius=360, coord_dim=coord_dim)
    ...
    def forward(self, data: Data) -> Tensor:
        ...
        '''
        if not self.pe_base == "rw":
            assert "coordinates" in data, "Trying to initialize geo-located positional encoding, but coordinates are not in Data object"
            base = data.coordinates.reshape(1, data.coordinates.shape[0], data.coordinates.shape[1])
        else:
            assert "rw_pe" in data, "Trying to initialize PE-GNN with random walk positional encoding, but random walk is not in Data object"
            base = data.rw_pe.reshape(1, data.rw_pe.shape[0], data.rw_pe.shape[1])
        emb = self.spatial_encoder(base.detach().cpu().numpy())
...
class SPE_Initializer(PE_Initializer):
    ...
    def __init__(self, pe_dim: int = 20, pe_hidden_dim: int = 32) -> None:
        ...
        self.eig_amt = 8 #TODO: make this the same variable (passed) as the one in the add_pe_to_func_ref function
        m = 4
    ...
    def calculate_eigs_per_graph(self, edge_index: Tensor, batch: Tensor | None, num_nodes: int):
        Lambda = [] # [B, eig_amt]
        V = [] # [N_sum, eig_amt]
        V_list = [] # [N_i, eig_amt] * B
        device = edge_index.device
        if batch == None: # if there is only one graph in the data
            eigenvalues, V = self.calculate_eigs(edge_index, num_nodes)
            Lambda = [eigenvalues]
            V_list = [torch.tensor(V, device=device)]
        else: # if there are multiple graphs in the data, treat each graph seperately.
            for graph_id in batch.unique(sorted=False):
                ...
                eigenvalues, eigenvectors = calculate_eigs(subgraph_edge_index, self.eig_amt, num_nodes)
                Lambda.append(eigenvalues)
                V_list.append(torch.tensor(eigenvectors, device=device))
        return Lambda, V_list
    ...
    def forward(self, data: Data) -> torch.Tensor:
        ...
        if data.batch is None:
            data.batch = torch.zeros(len(data.x), dtype=torch.int64, device=device)
        Lambda = [] # [B, eig_amt]
        V_list = [] # [N_i, eig_amt] * B

        if data.eigenvalues is not None and data.eigenvectors is not None:
            graph_amt = data.batch.max().item() + 1
            Lambda = data.eigenvalues.to(device)
            Lambda = Lambda.reshape(graph_amt, -1)
            V_list = [data.eigenvectors[data.batch==i] for i in data.batch.unique(sorted=False)]
        else:
            Lambda, V_list = self.calculate_eigs_per_graph(data.edge_index, data.batch, num_nodes=len(data.x))
            Lambda = torch.as_tensor(Lambda, device=device)
        ...
        for V, Z in zip(V_list, Z_list):   # [N_i, eig_amt] and [eig_amt, M]
            V = V.unsqueeze(dim=0).to(device)         # [1, N_i, eig_amt]
...
def get_pe_initializer(pe_dim: int = 20) -> PE_Initializer | None:
    ...
    elif pe_init == "pe-gnn-rw":
        return GeoPE_Initializer(pe_dim=pe_dim, pe_base="rw", coord_dim=pe_dim)
    elif pe_init == "spe":
