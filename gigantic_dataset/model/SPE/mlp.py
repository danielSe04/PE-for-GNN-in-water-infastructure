...
class MLP(nn.Module):
    ...
    def __init__(
        self, in_dims: int, out_dims: int, hidden_dims: int = 128, n_layers: int = 3, use_bn: bool = True, activation: str = "relu",
...
class MLPLayer(nn.Module):
    ...
    def forward(self, X: torch.Tensor) -> torch.Tensor:
        ...
        if self.bn is not None:
            ...
            if X.dim() > 2 or X.dim() == 2 and X.size(0) > 1:
                X = self.bn(X)                 # [prod(***), D_out]
            else:
                X = self.ln(X)                 # [prod(***), D_out]
